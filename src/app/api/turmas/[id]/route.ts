...
export async function PUT(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const existe = ehUuid(id)
      ? await db.turmas.findFirst({ where: { id, professorId: usuario.id } })
      : null;
    if (!existe) return erroApi("Turma não encontrada.", 404, "NAO_ENCONTRADO");
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  const db = await banco();
  if (!ehUuid(id)) return erroApi("Turma não encontrada.", 404, "NAO_ENCONTRADO");
