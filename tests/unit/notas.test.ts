// Testes das bibliotecas puras de notas (LaTeX, Markdown, aparência).
import { mkdirSync, writeFileSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import { describe, expect, it } from "vitest";
import { gerarTex, montarCreditos } from "@/lib/notas/render-latex";
import { analisarMarkdown, gerarMarkdown } from "@/lib/notas/render-markdown";
import { inlineParaLatex, prepararMatematicaTex, preprocessarLatex } from "@/lib/notas/latex";
import { normalizarAparencia, variaveisAparencia, type NotaDados } from "@/lib/notas/tipos";
import { DEMO_NOTA } from "@/lib/notas/demo";
import { notaModelo } from "@/lib/notas/modelo";

const aqui = dirname(fileURLToPath(import.meta.url));
const raiz = join(aqui, "..", "..");

function t(nome: string, cond: boolean): void {
  it(nome, () => {
    expect(cond).toBe(true);
  });
}

describe("bibliotecas de notas", () => {
  ...
  t(
    "imagem externa avisa endereço",
    texImg.includes("\\imagemexterna{https://exemplo.com/foto.png}"),
  );
  ...
  t(
    "negrito ** vira textbf",
    inlineParaLatex("oi **negrito** tchau").includes("\\textbf{negrito}"),
  );
  ...
  t(
    ...
    JSON.stringify(
      normalizarAparencia({ fonte: "lexend", escala: "gg", entrelinha: "compacta" }),
  ...
  it("grava .tex para compilação manual", () => {
